...
function formatPriceHistoryDateTime(value: string) {
...
function SelectedPropertyForm({
  ...
  const priceHistory = selectedProperty.priceHistory ?? []
  ...
  return (
    ...
    >
      ...
      <div className="specialized-field-row">
            ...
            <div className="specialized-history-list">
              {priceHistory.slice(0, 6).map((entry) => (
                <article key={entry.id} className="specialized-history-item">
                  ...
                  <time>{formatPriceHistoryDateTime(entry.changedAt)}</time>
                </article>
              ))}
            </div>
          ) : null}
      ...
      {activePurchase || selectedProperty.status === 'available' ? (
        <section className="specialized-property-action-card">
          <div className="specialized-property-action-card__header">
            <div>
              <strong>Client purchase</strong>
              <small>
                {activePurchase
                  ? selectedProperty.status === 'under_offer'
                    ? 'This property is under offer. Continue the active purchase until it is paid, cancelled, or expired.'
                    : 'Review the active client purchase and next payment steps.'
                  : 'Start the purchase workflow for this available property.'}
              </small>
            </div>
          </div>
          ...
          {activePurchase ? (
            ...
            >
              ...
              {purchaseStateLoading ? 'Checking purchase…' : 'Create Purchase'}
            </button>
          )}
        </section>
      ) : null}

      <div className="specialized-property-action-stack">
        <button
          className="specialized-btn specialized-btn-primary specialized-btn-block"
        ...
        {selectedEstate ? (
          <div className="specialized-kpi-grid">
            {[
              ...
              ['Under Offer', statsQuery.data?.underOffer],

...
export function mapPropertyPurchase(value: unknown): PropertyPurchase {
  ...
  return {
    ...
    canRequestPayment: Boolean(row.can_request_payment),
    hasLivePaymentRequest: Boolean(row.has_live_payment_request),
...
export const propertyPurchaseApi = {
  ...
  resendInvoiceEmail: async (purchaseId: number) =>
    mapPropertyPurchasePaymentRequest(
      await apiClient.post<unknown>(`/property-purchases/${purchaseId}/resend-invoice`, {}),
    ),

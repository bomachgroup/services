...
export function DeliverablesLivePage({ recordSearch }: { recordSearch: AppSectionSearch }) {
  ...
  const employeesQuery = useQuery({
    ...
    enabled:
      (Boolean(selectedOrderId) || createOpen) && hasPermission(user, PERMISSIONS.employeesList),
    retry: false,
  })
  ...
  const createMutation = useMutation({
    mutationFn: ({
      ...
      orderId: number
      input: Parameters<typeof deliverableApi.create>[1]
  ...
  return (
    ...
    >
        ...
        <section className="commercial-card">
          <header className="commercial-card-header">
            ...
            <div className="commercial-card-header-actions">
              ...
              <CompactActionButton
                disabled={!selectedOrderId || listQuery.isFetching}
          ...
          {!selectedOrderId ? (
            <EmptyState
              ...
              description={
                canListOrders
                  ? 'Select an Order to review its Deliverables. The backend does not yet expose a global Service Deliverable inbox endpoint.'
                  : 'Open Deliverables from a Service Order Control Room or request Service Order list access.'
              }
            />
          ) : !canViewOrders ? (
            <EmptyState
              title="Service Order access required"
              description="The Service Deliverable API requires orders.view to read Deliverables."
            />
            ...
            <ErrorState
              ...
              description={
                presentError(selectedOrderQuery.error ?? listQuery.error, 'section-load').message
              }
              onRetry={() => void refresh()}
            />
          ) : listQuery.data.items.length === 0 ? (
            <EmptyState
              title={hasFilters ? 'No Deliverables match these filters' : 'No Deliverables yet'}
              description={
                hasFilters
                  ? 'Change or clear the filters to review other Deliverables.'
                  : 'Add the first report, drawing, plan, certificate or handover output for this Service Order.'
              }
            />
          ) : (
                  ...
                  {listQuery.data.items.map((deliverable) => (
                    <tr key={deliverable.id}>
                      <td>
                        ...
                        <small>{deliverable.deliverableNumber}</small>
                      </td>
                      <td>
                        <b>
                          {deliverable.milestoneId
                            ? (milestoneNames.get(deliverable.milestoneId) ??
                              `Milestone #${deliverable.milestoneId}`)
                            : 'No milestone'}
                        </b>
                        <small>
                          {deliverable.taskId
                            ? (taskNames.get(deliverable.taskId) ?? `Task #${deliverable.taskId}`)
                            : 'No task link'}
                        </small>
                      </td>
                      ...
                      <td>
                        {deliverable.ownerId
                          ? (employeeNames.get(deliverable.ownerId) ??
                            `Employee #${deliverable.ownerId}`)
                          : 'Unassigned'}
                      </td>
          ...
          {selectedOrderId && listQuery.data ? (
            <div className="commercial-table-pagination">
              ...
              <div className="commercial-table-pagination-actions">
                <button
                  ...
                  className="commercial-btn commercial-btn-small"
                  disabled={page <= 1}
                  onClick={() => setSearchValue('page', page - 1)}
                >
                  Previous
                </button>
                <button
      ...
      {createOpen ? (
        <CreateDeliverableLiveWorkspace
      ...
      {selectedDeliverableId && detailQuery.isPending ? (
        <div className="commercial-modal-backdrop">
          <section className="commercial-modal">
            <div className="commercial-empty">Loading Deliverable…</div>
          </section>
        </div>
      ) : null}

      {selectedDeliverableId && detailQuery.isError ? (
        <div className="commercial-modal-backdrop">
          <section className="commercial-modal">
            <EmptyState
              title="Deliverable could not be opened"
      ...
      {detailQuery.data && selectedOrder ? (
        <DeliverableDetailLiveWorkspace
          key={`${detailQuery.data.id}-${detailQuery.data.updatedAt}`}
          deliverable={detailQuery.data}
          order={selectedOrder}
          ...
          onUpdate={(input) =>
            updateMutation.mutate({
              ...
              deliverableId: detailQuery.data.id,
              input,
            })
          }
          onApprove={() => {
            if (!canReviewDeliverable(detailQuery.data.status)) return
          ...
          onDelete={() => {
            if (!canDeleteDeliverable(detailQuery.data.status)) return
